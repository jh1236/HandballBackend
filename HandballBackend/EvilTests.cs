...
internal static class EvilTests {
    ...
    public static void MalevolantTest() {
        ...
        var db = new HandballContext();
        var games = db
            .Games.Where(g => !g.IsFinal && g.Ranked && !g.IsBye)
            .OrderBy(g => g.TournamentId == 2)
            .ThenBy(g => g.TournamentId == 3)
            .ThenBy(g => g.StartTime)
            .ThenBy(g => g.Id)
            .IncludeRelevant()
            .Include(g => g.Events.Where(gE => gE.EventType == GameEventType.Forfeit));
        foreach (var game in games) {
            ...
            var playingPlayers = game
                .Players.Where(pgs =>
                    (
                        ...
                        || pgs.RoundsCarded + pgs.RoundsOnCourt > 0
                    )
                )
                .ToList();
            var teamOneElo = teamElos.GetValueOrDefault(
                game.TeamOne.Id,
                playingPlayers
            ...
            var teamTwoElo = teamElos.GetValueOrDefault(
                game.TeamTwo.Id,
                playingPlayers
            ...
            foreach (var pgs in game.Players) {
                ...
                var eloDelta = EloCalculator.CalculateEloDelta(
                    myElo,
                    oppElo,
    ...
    public static void SinisterTest() {
        init();
        var teams = new[] { 62, 126, 116, 127, 128, 129, 131, 132, 133, 134, 136 };
    ...
    public static Tournament CreateTournament(
        ...
        foreach (var teamId in teams.OrderBy(i => i)) {
            db.TournamentTeams.Add(
                new TournamentTeam { TournamentId = tournament.Id, TeamId = teamId }
            );
        }

        foreach (var officialId in officials.OrderBy(i => i)) {
            db.TournamentOfficials.Add(
                new TournamentOfficial {
    ...
    public static void NefariousTest() {
        ...
        TextHelper
            .Text(digby, "And now the api key isnt even in the code!!")
            .GetAwaiter()
            .GetResult();
    }
    ...
    public static void DeviousTest() {
        ...
        while (game != null) {
            ...
            GameManager.End(i, null, 3, 3, "Testing", null, null, "", "", false);
            i++;
            game = db.Games.FirstOrDefault(g => g.GameNumber == i);
        }
    }
    ...
    public static void WickedTest() {
        ...
        var db = new HandballContext();
        var people = db
            .TournamentTeams.Where(tt => tt.TournamentId == 10)
            .IncludeRelevant()
            .Select(t => t.Team)
            .ToArray()
            .SelectMany(t => t.People)
            .ToList();
    ...
    public static void FixImages() {
        ...
        var teams = db.Teams.Where(t =>
            t.NonCaptainId != null && (t.ImageUrl == null || !t.ImageUrl.StartsWith("/"))
        );
    ...
    public static void VotesFixerer() {
        init();
        var db = new HandballContext();
        var gamesToFix = db.Games.Where(g =>
            g.Ended && !g.IsBye && g.Events.All(gE => gE.EventType != GameEventType.Votes)
        );
        var prevGame = -1;
        foreach (var game in gamesToFix.Include(game => game.Events).Include(game => game.Players)) {
            ...
            if (bestPlayerId is null)
                continue;
            if (
                !game.Ranked
                || (
                    endEvent.TeamOneLeftId == endEvent.TeamOneRightId
            ...
            if (game.Players.All(pgs => pgs.PlayerId != bestPlayerId)) {
                Console.WriteLine(
                    $"WTF?!: game {game.GameNumber} has players "
            ...
            var newEvent = GameManager.SetUpGameEvent(
                game,
                GameEventType.Votes,
                pgs.TeamId == game.TeamOneId,
                pgs.PlayerId,
                details: 2
            );
