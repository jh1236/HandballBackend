...
def sync(game_id):
    ...
    for j in all_players:
        if not events:
            j.side_of_court = j.start_side
        elif j.player_id in [i.team_one_left_id, i.team_two_left_id]:
            j.side_of_court = 'Left'
        elif j.player_id in [i.team_one_right_id, i.team_two_right_id]:
            j.side_of_court = 'Right'
        else:
            j.side_of_court = 'Substitute'
...
def start_game(game_id, swap_service, team_one, team_two, team_one_iga, official=None, scorer=None):
    ...
    for id, side in zip(team_one, SIDES):
        ...
        PlayerGameStats.query.filter(PlayerGameStats.id == id).first().side_of_court = side
        ...
        if players is not None:
            ...
            if not out_team:
                if len([i for i in new_players if i > 0]) == 1:
                    team = "(Solo) " + People.query.filter(People.id == new_players[0]).first().name
                if not team:
